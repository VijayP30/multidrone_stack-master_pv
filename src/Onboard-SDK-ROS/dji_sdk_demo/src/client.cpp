...
void printSuccess(bool wasSuccessful) {
    if (wasSuccessful) {
...
int main(int argc, char **argv)
{
    ...
    while(1)
    {
		ros::spinOnce();
        temp32 = getchar();
        if(temp32 != 10) // check that not newline character
        {
            if(valid_flag == false)
            {
        ...
        switch(main_operate_code)
        {
			case 'a':
				/* SDK version query*/
				printSuccess(drone->check_version());
				break;
            case 'b':
                /* request control ability*/ /////////
                printSuccess(drone->request_sdk_permission_control());
                break;
            case 'c':
                /* release control ability*/ /////////
                printSuccess(drone->release_sdk_permission_control());
                break;
            case 'd':
                /* take off */ ////////
                printSuccess(drone->takeoff());
                break;
            case 'e':
                /* landing*/ /////////
                printSuccess(drone->landing());
                break;
            case 'f':
                /* go home*/
                printSuccess(drone->gohome());
                break;
            case 'g':
                /*gimbal test*/
		...
		for(int j = 0; j < 1000; j ++)
            ...
            case 'k':
                /*take a picture*/
                printSuccess(drone->take_picture());
                break;
            case 'l':
                /*start video*/
                printSuccess(drone->start_video());
                break;
            case 'm':
                /*stop video*/
                printSuccess(drone->stop_video());
                break;
            case 'n':
                /* Local Navi Test */
			...
			case 'q':
				//drone arm
				printSuccess(drone->drone_arm());
                break;
			case 'r':
				//drone disarm
				printSuccess(drone->drone_disarm());
                break;
			case 's':
			...
			case 't':
				//sync flag
				printSuccess(drone->sync_flag_control(1));
				break;
			case 'u':
				//set msg frequency
				printSuccess(drone->set_message_frequency(msg_frequency_data));
				break;
			...
			case 'v':
                ...
                */
				...
				printSuccess(drone->mission_waypoint_upload(waypoint_task));
				break;
			case 'w':
				...
				printSuccess(drone->mission_hotpoint_upload(hotpoint_task));
				break;
			case 'x':
				...
				printSuccess(drone->mission_followme_upload(followme_task));
				break;
			case 'y':
				//mission start
				printSuccess(drone->mission_start());
				break;
			case 'z':
				//mission pause
				printSuccess(drone->mission_pause());
				break;
			case '1':
				//mission resume
				printSuccess(drone->mission_resume());
				break;
			case '2':
				//mission cancel
				printSuccess(drone->mission_cancel());
				break;
			case '3':
			...
			case '8':
				...
				printSuccess(drone->mission_hotpoint_reset_yaw());
				break;
			case '9':
